...
function CardBack({ className = "" }: { className?: string }) {
  return (
    <div className={`uno-shell ${className}`} style={{ ["--cw" as string]: "var(--card-md)" }}>
...
export default function Game() {
  ...
  return (
    <div className="felt-table felt-rail h-[100dvh] w-full flex flex-col overflow-hidden select-none text-[#ece6da] safe-top safe-bottom">
      {/* ===== Top bar ===== */}
      <header className="flex items-center justify-between px-2 sm:px-3 py-2 z-20 flex-shrink-0">
        <div className="flex items-center gap-2">
          <button onClick={handleLeave} className="p-2 rounded-full bg-black/35 border border-white/10 text-gray-200 hover:text-white touch-target"><LogOut className="w-4 h-4" /></button>
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setSoundEnabled(!soundEnabled)} className="p-2 rounded-full bg-black/35 border border-white/10 text-gray-200 hover:text-white touch-target">{soundEnabled ? <Volume2 className="w-4 h-4" /> : <VolumeX className="w-4 h-4 text-gray-500" />}</button>
          <button onClick={() => { setShowChat(true); }} className="p-2 rounded-full bg-black/35 border border-white/10 text-gray-200 hover:text-white touch-target"><Smile className="w-4 h-4" /></button>
          <button onClick={() => setShowChat(!showChat)} className="p-2 rounded-full bg-black/35 border border-white/10 text-gray-200 hover:text-white relative touch-target">
      ...
      {/* ===== Opponents ===== */}
      <div className="flex justify-center items-start gap-2 sm:gap-3 md:gap-6 lg:gap-10 px-2 pt-1 flex-shrink-0 flex-wrap">
          ...
          return (
            <motion.div key={player.id} initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.07 }} className="flex flex-col items-center gap-1">
              ...
              <div className="relative mt-0.5">
                <TimerRing pct={timerPct} active={isTurn} size={ringSize} />
                <div className={`w-[${avatarSize}px] h-[${avatarSize}px] ${isTurn ? "frame-ring frame-ring-turn" : "frame-ring"}`} style={{ width: avatarSize, height: avatarSize }}>
              ...
              <span className="text-[10px] sm:text-[11px] font-bold truncate max-w-[60px] sm:max-w-[72px]">{player.username}</span>
      ...
      <div className="flex-1 flex items-center justify-center relative min-h-0 px-2">
        {/* direction arcs */}
        <div className={`absolute w-48 h-48 sm:w-64 sm:h-64 md:w-80 md:h-80 rounded-full border-2 border-dashed border-gold/20 animate-dir-flow ${store.direction === "counter_clockwise" ? "scale-x-[-1]" : ""}`} />
        <svg className={`absolute w-56 h-56 sm:w-72 sm:h-72 md:w-96 md:h-96 animate-dir-flow ${store.direction === "counter_clockwise" ? "scale-x-[-1]" : ""}`} viewBox="0 0 200 200" fill="none">
        ...
        <div className="flex items-center gap-3 sm:gap-4 md:gap-6 relative z-10">
            ...
            <AnimatePresence mode="popLayout">
              {store.topCard && (
                <motion.div key={store.topCard.id} initial={anims ? { scale: 0.5, rotate: -24, y: 120, opacity: 0 } : false} animate={{ scale: anims ? [0.5, 1.14, 1] : 1, rotate: anims ? [-24, 7, 0] : 0, y: anims ? [120, -10, 0] : 0, opacity: 1 }} exit={{ opacity: 0, scale: 0.8 }} transition={{ duration: anims ? 0.42 : 0, times: [0, 0.68, 1], ease: "easeOut" }}>
                  <UnoCard card={store.topCard} size={cardSize} active />
                </motion.div>
              )}
            </AnimatePresence>
            <AnimatePresence mode="wait">
              {lastAction && (
                <motion.div key={lastAction} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="absolute -bottom-7 left-1/2 -translate-x-1/2 whitespace-nowrap text-[10px] sm:text-[11px] font-bold text-gold bg-black/50 px-3 py-1 rounded-full">{lastAction}</motion.div>
              )}
            </AnimatePresence>
          </div>
          ...
          <div className="relative flex flex-col items-center">
            <div className="w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 wild-wheel animate-wheel-spin" />
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-5 h-5 sm:w-6 sm:h-6 rounded-full border-2 border-white shadow" style={{ backgroundColor: SUIT_HEX[store.activeColor] || "#fff" }} />
            <span className="mt-1.5 text-[8px] sm:text-[9px] font-bold text-gold/80 bg-black/40 px-2 py-0.5 rounded-full">COLOR</span>
          </div>
        </div>
        ...
        <AnimatePresence>
          {store.showColorPicker && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 flex items-center justify-center bg-black/55 backdrop-blur-sm z-30">
              <motion.div initial={{ scale: 0.85, y: 10 }} animate={{ scale: 1, y: 0 }} className="glass rounded-3xl p-4 sm:p-6">
                <h3 className="font-display font-bold text-base sm:text-lg text-center mb-4">Choose a Color</h3>
      ...
      <div className="flex items-center justify-center gap-3 flex-shrink-0 py-1 relative">
        {settings.tutorialHints && store.isMyTurn && !hasPlayable && store.myHand.length > 0 && (
          <span className="text-[10px] sm:text-[11px] font-bold text-gold bg-black/40 px-3 py-1 rounded-full">No moves — draw a card</span>
        )}
        <AnimatePresence>
          {store.isMyTurn && (
            <motion.span initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="text-[10px] sm:text-[11px] font-bold text-emerald-300 bg-black/40 px-3 py-1 rounded-full">YOUR TURN</motion.span>
          )}
        </AnimatePresence>
        <motion.button whileTap={{ scale: 0.94 }} onClick={handleUno} disabled={store.myHand.length !== 1 || me?.declaredUno}
          className={`uno-btn w-14 h-14 sm:w-16 sm:h-16 rounded-full font-display font-extrabold text-base sm:text-lg ${store.myHand.length === 1 && !me?.declaredUno ? "animate-glow-pulse" : ""}`}>
          UNO!
        </motion.button>
      </div>
      ...
      <div className="flex-shrink-0 px-2 pb-3 pt-1">
        <div className="flex items-center justify-center gap-2 mb-1">
          <div className="relative frame-ring w-6 h-6 sm:w-7 sm:h-7"><img src={me?.avatar || "https://i.pravatar.cc/200?img=12"} alt="" className="w-full h-full rounded-[0.5rem] object-cover" /></div>
          <span className="text-[11px] sm:text-xs font-bold">{me?.username || "You"}</span>
          <span className="count-badge text-[9px] sm:text-[10px]">{store.myHand.length}</span>
        </div>
        <div className="flex justify-center overflow-x-auto pb-1" style={{ minHeight: isMobile ? "82px" : "108px" }}>
          <div className="flex items-end" style={{ paddingLeft: `${handPad}px` }}>
            {store.myHand.map((card, index) => {
              ...
              return (
                <motion.div key={card.id} initial={anims ? { y: 90, x: -140, opacity: 0, rotate: -14, scale: 0.8 } : false} animate={{ y: playable ? -8 : 0, x: 0, opacity: 1, rotate: 0, scale: 1 }} transition={{ delay: anims ? index * 0.045 : 0, type: "spring", stiffness: 240, damping: 20 }}
                  className="flex-shrink-0 relative z-0 hover:z-10" style={{ marginLeft: `-${overlap}px` }}>
                  <UnoCard card={card} size={handCardSize} playable={playable} onClick={playable ? () => handlePlayCard(index) : undefined} className={playable ? "" : "opacity-75 saturate-[0.85]"} />
                </motion.div>
              );
            })}
          </div>
      ...
      <AnimatePresence>
        {showChat && (
          <motion.div initial={{ opacity: 0, x: 300 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 300 }} className="fixed right-0 top-0 bottom-0 w-64 sm:w-72 glass border-l border-white/10 z-40 flex flex-col">
        ...
        {unoCall && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[55] flex items-center justify-center pointer-events-none">
            ...
            <motion.div initial={{ scale: 0, rotate: -18 }} animate={{ scale: [0, 1.25, 1], rotate: [-18, 6, 0] }} transition={{ duration: 0.6, ease: "easeOut" }} className="relative text-center">
              <h1 className="font-display font-extrabold text-7xl sm:text-8xl md:text-9xl" style={{ color: "#ffce3a", textShadow: "0 0 40px rgba(224,30,30,0.9), 0 6px 0 #a81212, 0 10px 20px rgba(0,0,0,0.7)" }}>UNO<span className="text-unored">!</span></h1>
              <motion.p initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.3 }} className="font-display font-extrabold text-2xl sm:text-3xl text-emerald-400 mt-2" style={{ textShadow: "0 0 20px rgba(67,209,102,0.7)" }}>+100</motion.p>
            </motion.div>
            <Confetti count={50} />
          </motion.div>
        )}
      ...
      <AnimatePresence>
        {store.phase === "round_end" && store.roundScores && (
          ...
          <Overlay>
            <h2 className="font-display text-xl sm:text-2xl font-extrabold text-center text-gold glow-gold-text mb-1">Round Over!</h2>
      ...
      <AnimatePresence>
        {store.phase === "match_end" && store.finalScores && (
          ...
          <Overlay gold>
            <motion.div initial={{ scale: 0, rotate: -20 }} animate={{ scale: 1, rotate: 0, y: [0, -6, 0] }} transition={{ scale: { type: "spring", stiffness: 200 }, y: { duration: 2, repeat: Infinity } }} className="text-6xl sm:text-7xl text-center mb-1" style={{ filter: "drop-shadow(0 0 20px rgba(255,209,92,0.7))" }}>🏆</motion.div>
            <motion.div initial={{ scale: 0.6, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: 0.2 }}
              className="mx-auto mb-4 px-5 py-1.5 rounded-xl text-center font-display font-extrabold text-xl sm:text-2xl text-white w-fit"
            ...
            <div className="grid grid-cols-3 gap-2 mb-4">
              {[{ icon: "🪙", v: "+500", c: "#ffd255" }, { icon: "✦", v: "+250", c: "#a356f0" }, { icon: "🎁", v: "+1", c: "#f0a818" }].map((r, i) => (
                <motion.div key={i} initial={{ y: 16, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.3 + i * 0.1 }} className="panel-inset rounded-xl p-2 sm:p-3 text-center">
                  <div className="text-xl sm:text-2xl mb-1" style={{ color: r.c }}>{r.icon}</div>
...
function Overlay({ children, gold }: { children: ReactNode; gold?: boolean }) {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <motion.div initial={{ scale: 0.85, y: 20 }} animate={{ scale: 1, y: 0 }} transition={{ type: "spring", stiffness: 260, damping: 22 }} className={`glass rounded-3xl p-4 sm:p-6 max-w-xs w-full ${gold ? "border border-gold/40" : ""}`}>
        {children}
      </motion.div>
    </motion.div>
  );
